...
function mostCostEffective (bubbleScores, costsArr, bestArr) {
  var starCost = bestArr[0];
  for ( var i = 0; bestArr > i; i++){
    if (costsArr[bestArr[i]] > starCost) {
      starCost = bubbleScores[bestArr[i]];
    }
  }
  document.write("<br> Best cost on position : " + starCost + ". " + costsArr[bestArr[i]] + "is the most effective cost.");
}
